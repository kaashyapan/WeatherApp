...
type Datastar(ctx: HttpContext) =
    ...
    member this.WriteHtmlFragment(htmlView, ?options) =
    ...
    member this.RemoveHtmlFragment(selector, ?options) =
        ServerSentEventGenerator.removeFragments (_sse, selector, ?options = options)

    member this.MergeSignal(signals, ?options, ?jsonSerializerOptions: JsonSerializerOptions) =
        let json =
    ...
    member this.RemoveSignal(paths, ?options) = ServerSentEventGenerator.removeSignals (_sse, paths, ?options = options)

    member this.ExecuteScript(script, ?options) =
